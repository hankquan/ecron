...
// FlushHistoryCache flushes the history cache to a history file and logs the operation.
// This function creates a history directory under the user's home directory, writes the current cache content to a history file,
// and logs the history operation in the history.log file.
...
func FlushHistoryCache() {
	...
	backupDir := filepath.Join(historyDir, "history")
